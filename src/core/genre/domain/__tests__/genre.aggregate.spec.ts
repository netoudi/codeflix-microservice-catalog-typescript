...
describe('Genre Without Validator Unit Tests', () => {
  ...
  test('constructor of genre', () => {
    ...
    expect(genre.categories).toEqual(new Map([[nestedCategory.category_id.id, nestedCategory]]));
  ...
  describe('create command', () => {
    test('should create a genre', () => {
      ...
      const name = 'Movie';
      const nestedCategory = Category.fake().aNestedCategory().build();
      const categoriesId = new Map([[nestedCategory.category_id.id, nestedCategory]]);
      const is_active = true;
      const created_at = new Date();
