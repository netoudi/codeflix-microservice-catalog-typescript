...
describe('GenreElasticSearchRepository Integration Tests', () => {
  ...
  test('should insert many entities', async () => {
    ...
    const { exists: foundGenres } = await repository.findByIds(genres.map((g) => g.genre_id));
  ...
  it('should finds a entity by id', async () => {
    ...
    await expect(repository.ignoreSoftDeleted().findById(entity.genre_id)).resolves.toBeNull();
  });
  ...
  it('should return a genres list by ids', async () => {
    ...
    Promise.all([await repository.update(genres[0]), await repository.update(genres[1])]);

    const { exists: foundGenres2 } = await repository.ignoreSoftDeleted().findByIds(genres.map((g) => g.genre_id));
  ...
  it('should return genre id that exists', async () => {
    ...
    const existsResult2 = await repository.existsById([genre.genre_id, genreId1]);
    ...
    const existsResult3 = await repository.ignoreSoftDeleted().existsById([genre.genre_id]);
  ...
  it('should throw error on update when a entity not found', async () => {
    const entity = Genre.fake().aGenre().build();
    await expect(repository.update(entity)).rejects.toThrow(new NotFoundError(entity.genre_id.id, Genre));
  ...
  it('should throw error on delete when a entity not found', async () => {
    ...
    await expect(repository.delete(genreId)).rejects.toThrow(new NotFoundError(genreId.id, Genre));
    ...
    await expect(repository.ignoreSoftDeleted().delete(entity.genre_id)).rejects.toThrow(
      new NotFoundError(entity.genre_id.id, Genre),
    );
  });
