...
import { CastMemberId, CastMember } from '@/core/cast-member/domain/cast-member.aggregate';
...
export class CastMemberFakeBuilder<TBuild = any> {
  ...
  private _cast_member_id: PropOrFactory<CastMemberId> = (_index) => new CastMemberId();
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  private constructor(countObjs: number = 1, mode = CastMemberFakeMode.ONLY_AGGREGATE) {
  ...
  static aDirector() {
    return new CastMemberFakeBuilder<CastMember>().withType(CastMemberType.createADirector());
  }

  static aNestedDirector() {
    return new CastMemberFakeBuilder<NestedCastMember>(1, CastMemberFakeMode.ONLY_NESTED).withType(
      CastMemberType.createADirector(),
    );
  }

  static aDirectorAndNested() {
    return new CastMemberFakeBuilder<[CastMember, NestedCastMember]>(1, CastMemberFakeMode.BOTH);
  }

  static anActor() {
    return new CastMemberFakeBuilder<CastMember>().withType(CastMemberType.createAnActor());
  }
  ...
  static theActors(countObjs: number) {
    return new CastMemberFakeBuilder<CastMember[]>(countObjs).withType(CastMemberType.createAnActor());
  }

  static theActorsAndNested(countObjs: number) {
    return new CastMemberFakeBuilder<[CastMember, NestedCastMember][]>(countObjs, CastMemberFakeMode.BOTH);
  }

  static theNestedActors(countObjs: number) {
    return new CastMemberFakeBuilder<NestedCastMember[]>(countObjs, CastMemberFakeMode.ONLY_NESTED).withType(
      CastMemberType.createAnActor(),
    );
  }

  static theDirectors(countObjs: number) {
    return new CastMemberFakeBuilder<CastMember[]>(countObjs).withType(CastMemberType.createADirector());
  }
