...
export class CastMemberElasticSearchMapper {
  static toEntity(id: string, document: CastMemberDocument): CastMember {
    ...
    const [type, errorCastMemberType] = CastMemberType.create(document.cast_member_type as any).asArray();
...
export class CastMemberElasticSearchRepository implements ICastMemberRepository {
  constructor(
  ...
  sortableFieldsMap: { [key: string]: string } = { name: 'cast_member_name', created_at: 'created_at' };
  ...
  async findAll(): Promise<CastMember[]> {
    ...
    return result.body.hits.hits.map((hit) => CastMemberElasticSearchMapper.toEntity(hit._id, hit._source));
  }

  async findByIds(ids: CastMemberId[]): Promise<{ exists: CastMember[]; not_exists: CastMemberId[] }> {
    const query = {
      bool: {
        must: [
          {
    ...
    const docs = result.body.hits.hits as GetGetResult<CastMemberDocument>[];
    return {
      exists: docs.map((doc) => CastMemberElasticSearchMapper.toEntity(doc._id as string, doc._source!)),
      not_exists: ids.filter((id) => !docs.some((doc) => doc._id === id.id)),
    };
  }

  async findOneBy(filter: { cast_member_id?: CastMemberId }): Promise<CastMember | null> {
    ...
    return CastMemberElasticSearchMapper.toEntity(docs[0]._id as string, document);
  }
  ...
  async existsById(ids: CastMemberId[]): Promise<{ exists: CastMemberId[]; not_exists: CastMemberId[] }> {
    ...
    const docs = result.body.hits.hits as GetGetResult<CastMemberDocument>[];
    const existsCastMemberIds = docs.map((m) => new CastMemberId(m._id as string));
  ...
  async search(props: CastMemberSearchParams): Promise<CastMemberSearchResult> {
    ...
    const docs = result.body.hits.hits as GetGetResult<CastMemberDocument>[];
    const entities = docs.map((doc) => CastMemberElasticSearchMapper.toEntity(doc._id as string, doc._source!));
