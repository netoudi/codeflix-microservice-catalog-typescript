...
} from '@/core/cast-member/infra/db/elastic-search/cast-member-elastic-search';
...
describe('CastMemberElasticSearchRepository Integration Tests', () => {
  ...
  test('should insert many entities', async () => {
    ...
    const { exists: foundCastMembers } = await repository.findByIds(castMembers.map((g) => g.cast_member_id));
  ...
  it('should finds a entity by id', async () => {
    ...
    await expect(repository.ignoreSoftDeleted().findById(entity.cast_member_id)).resolves.toBeNull();
  });
  ...
  it('should return a cast members list by ids', async () => {
    ...
    Promise.all([await repository.update(castMembers[0]), await repository.update(castMembers[1])]);
  ...
  it('should return cast member id that exists', async () => {
    ...
    await repository.insert(castMember);
    const existsResult1 = await repository.existsById([castMember.cast_member_id]);
    ...
    const notExistsResult = await repository.existsById([castMemberId1, castMemberId2]);
    ...
    const existsResult2 = await repository.existsById([castMember.cast_member_id, castMemberId1]);
    ...
    const existsResult3 = await repository.ignoreSoftDeleted().existsById([castMember.cast_member_id]);
    expect(existsResult3.exists).toHaveLength(0);
    expect(existsResult3.not_exists).toHaveLength(1);
    expect(existsResult3.not_exists[0]).toBeValueObject(castMember.cast_member_id);
  });

  it('should throw error on update when a entity not found', async () => {
    const entity = CastMember.fake().aDirector().build();
    await expect(repository.update(entity)).rejects.toThrow(new NotFoundError(entity.cast_member_id.id, CastMember));
  ...
  it('should throw error on delete when a entity not found', async () => {
    ...
    await expect(repository.delete(castMemberId)).rejects.toThrow(new NotFoundError(castMemberId.id, CastMember));
    ...
    await expect(repository.ignoreSoftDeleted().delete(entity.cast_member_id)).rejects.toThrow(
      new NotFoundError(entity.cast_member_id.id, CastMember),
    );
  });
  ...
  it('should delete a entity', async () => {
    ...
    expect(document.body.hits.hits.length).toBe(0);
    ...
    expect(document2.body.hits.hits.length).toBe(0);
  });
  ...
  describe('search method tests', () => {
    it('should only apply paginate and order by created desc when other params are null', async () => {
      ...
      const searchOutput2 = await repository.ignoreSoftDeleted().search(CastMemberSearchParams.create());
    ...
    it('should apply paginate and filter by type', async () => {
      ...
      const castMembers = [
        CastMember.fake().anActor().withName('actor1').withCreatedAt(created_at).build(),
        CastMember.fake().anActor().withName('actor2').withCreatedAt(created_at).build(),
        CastMember.fake().anActor().withName('actor3').withCreatedAt(created_at).build(),
        CastMember.fake().aDirector().withName('director1').withCreatedAt(created_at).build(),
        CastMember.fake().aDirector().withName('director2').withCreatedAt(created_at).build(),
        CastMember.fake().aDirector().withName('director3').withCreatedAt(created_at).build(),
      ];
      await repository.bulkInsert(castMembers);
      ...
      for (const arrangeItem of arrange) {
        ...
        searchOutput.items.forEach((item, key) => {
          ...
          expect(item.toJSON()).toStrictEqual(expect.objectContaining(expected));
        });
      }
    });
