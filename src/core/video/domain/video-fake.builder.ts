...
export class VideoFakeBuilder<TBuild = any> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private _banner_url: PropOrFactory<string | null> | undefined = (_index) => 'videos/images/banner.jpg';
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private _thumbnail_url: PropOrFactory<string | null> | undefined = (_index) => 'videos/images/thumbnail.jpg';
  private _thumbnail_half_url: PropOrFactory<string | null> | undefined = (
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _index,
  ) => 'videos/images/thumbnail_half.jpg';
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private _trailer_url: PropOrFactory<string | null> | undefined = (_index) => 'videos/trailers';
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private _video_url: PropOrFactory<string | null> | undefined = (_index) => 'videos/videos';
  ...
  static aVideoWithoutImageMedias() {
    return new VideoFakeBuilder<Video>().withoutBannerUrl().withoutThumbnailUrl().withoutThumbnailHalfUrl();
  }
  ...
  static theVideosWithoutImageMedias(countObjs: number) {
    return new VideoFakeBuilder<Video[]>(countObjs).withoutBannerUrl().withoutThumbnailUrl().withoutThumbnailHalfUrl();
  }
  ...
  build(): TBuild {
    const videos = new Array(this.countObjs).fill(undefined).map((_, index) => {
      ...
      const nestedGenres = this._genres.length ? this.callFactory(this._genres, index) : [nestedGenre];
      ...
      const video = new Video({
        ...
        cast_members: new Map(nestedCastMembers.map((nested) => [nested.cast_member_id.id, nested])),
        genres: new Map(nestedGenres.map((nested) => [nested.genre_id.id, nested])),
