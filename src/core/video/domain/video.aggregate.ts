...
} from '@/core/cast-member/domain/nested-cast-member.entity';
...
import { NestedGenre, NestedGenreConstructorProps } from '@/core/genre/domain/nested-genre.entity';
...
export class Video extends AggregateRoot {
  ...
  static create(props: VideoCreateCommand) {
    const genre = new Video({
      ...
      genres: new Map(
        props.genres_props.map((genre_props) => [genre_props.genre_id.id, NestedGenre.create(genre_props)]),
      ),
  ...
  syncNestedGenres(genresProps: NestedGenreConstructorProps[]) {
    ...
    this.genres = new Map(genresProps.map((genreProps) => [genreProps.genre_id.id, NestedGenre.create(genreProps)]));
  }
  ...
  toJSON() {
    return {
      ...
      thumbnail_half_url: this.thumbnail_half_url ? this.thumbnail_half_url : null,
      trailer_url: this.trailer_url,
      video_url: this.video_url,
      categories: Array.from(this.categories.values()).map((category) => category.toJSON()),
      genres: Array.from(this.genres.values()).map((genre) => genre.toJSON()),
      cast_members: Array.from(this.cast_members.values()).map((cast_member) => cast_member.toJSON()),
