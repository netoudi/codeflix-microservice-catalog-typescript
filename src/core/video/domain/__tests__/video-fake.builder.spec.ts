...
describe('VideoFakerBuilder Unit Tests', () => {
  ...
  describe('thumbnail_half_url prop', () => {
    ...
    test('should pass index to thumbnail_half_url factory', () => {
      ...
      const fakerMany = VideoFakeBuilder.theVideosWithoutImageMedias(2);
      fakerMany.withThumbnailHalfUrl((index) => `test thumbnail_half_url ${index}`);
      const categories = fakerMany.build();

      expect(categories[0].thumbnail_half_url).toBe(`test thumbnail_half_url 0`);
      expect(categories[1].thumbnail_half_url).toBe(`test thumbnail_half_url 1`);
    });
  });
  ...
  describe('categories prop', () => {
    ...
    it('should pass index to categories factory', () => {
      const nestedCategories = [Category.fake().aNestedCategory().build(), Category.fake().aNestedCategory().build()];
      faker.addNestedCategory((index) => nestedCategories[index]);
      const genre = faker.build();

      expect(genre.categories.get(nestedCategories[0].category_id.id)).toBe(nestedCategories[0]);

      const fakerMany = VideoFakeBuilder.theVideosWithoutImageMedias(2);
      fakerMany.addNestedCategory((index) => nestedCategories[index]);
      const genres = fakerMany.build();

      expect(genres[0].categories.get(nestedCategories[0].category_id.id)).toBe(nestedCategories[0]);

      expect(genres[1].categories.get(nestedCategories[1].category_id.id)).toBe(nestedCategories[1]);
    });
  });
  ...
  describe('genres prop', () => {
    ...
    it('should pass index to genres factory', () => {
      const nestedGenres = [Genre.fake().aNestedGenre().build(), Genre.fake().aNestedGenre().build()];
      faker.addNestedGenre((index) => nestedGenres[index]);
      const genre = faker.build();

      expect(genre.genres.get(nestedGenres[0].genre_id.id)).toBe(nestedGenres[0]);

      const fakerMany = VideoFakeBuilder.theVideosWithoutImageMedias(2);
      fakerMany.addNestedGenre((index) => nestedGenres[index]);
      const genres = fakerMany.build();

      expect(genres[0].genres.get(nestedGenres[0].genre_id.id)).toBe(nestedGenres[0]);

      expect(genres[1].genres.get(nestedGenres[1].genre_id.id)).toBe(nestedGenres[1]);
    });
  });
  ...
  describe('cast_members prop', () => {
    ...
    it('should pass index to cast_members factory', () => {
      const nestedCastMembers = [CastMember.fake().aNestedActor().build(), CastMember.fake().aNestedActor().build()];
      faker.addNestedCastMember((index) => nestedCastMembers[index]);
      const genre = faker.build();

      expect(genre.cast_members.get(nestedCastMembers[0].cast_member_id.id)).toBe(nestedCastMembers[0]);

      const fakerMany = VideoFakeBuilder.theVideosWithoutImageMedias(2);
      fakerMany.addNestedCastMember((index) => nestedCastMembers[index]);
      const castMembers = fakerMany.build();

      expect(castMembers[0].cast_members.get(nestedCastMembers[0].cast_member_id.id)).toBe(nestedCastMembers[0]);

      expect(castMembers[1].cast_members.get(nestedCastMembers[1].cast_member_id.id)).toBe(nestedCastMembers[1]);
    });
  });
  ...
  it('should create a video', () => {
    ...
    expect(video.categories.values().next().value).toBeInstanceOf(NestedCategory);
    ...
    expect(video.cast_members.values().next().value).toBeInstanceOf(NestedCastMember);
    ...
    expect(video.categories.get(nestedCategory1.category_id.id)).toBe(nestedCategory1);
    expect(video.categories.get(nestedCategory2.category_id.id)).toBe(nestedCategory2);
    ...
    expect(video.cast_members.get(nestedCastMember1.cast_member_id.id)).toBe(nestedCastMember1);
    expect(video.cast_members.get(nestedCastMember2.cast_member_id.id)).toBe(nestedCastMember2);
