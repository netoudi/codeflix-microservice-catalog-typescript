...
function createVideo() {
  ...
  return new Video({
    ...
    cast_members: new Map([[nestedCastMember.cast_member_id.id, nestedCastMember]]),
    created_at,
  });
}
...
describe('Video Without Validator Unit Tests', () => {
  ...
  test('constructor of video', () => {
    ...
    expect(video.categories).toEqual(new Map([[nestedCategory.category_id.id, nestedCategory]]));
    expect(video.genres).toEqual(new Map([[nestedGenre.genre_id.id, nestedGenre]]));
    expect(video.cast_members).toEqual(new Map([[nestedCastMember.cast_member_id.id, nestedCastMember]]));
...
describe('Video Validator', () => {
  describe('create command', () => {
    test('should an invalid video with title property', () => {
      const video = Video.fake().aVideoWithoutImageMedias().withInvalidTitleTooLong().build();
