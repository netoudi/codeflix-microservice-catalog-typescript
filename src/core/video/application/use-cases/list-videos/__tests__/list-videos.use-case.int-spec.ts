...
describe('ListVideosUseCase Integration Tests', () => {
  ...
  it('should return output sorted by created_at when input param is empty', async () => {
    ...
    expect(output.items.map((i) => i.title)).toEqual(['title2', 'title1', 'title0']);
  });
  ...
  it('should return output filter by categories_id, genres_id and cast_members_id', async () => {
    ...
    const output = await useCase.execute({
      filter: {
        categories_id: [videos[0].categories.values().next().value.category_id.id],
        genres_id: [videos[0].genres.values().next().value.genre_id.id],
        cast_members_id: [videos[0].cast_members.values().next().value.cast_member_id.id],
      },
    });
