...
export class SaveVideoUseCase implements IUseCase<SaveVideoInput, SaveVideoOutput> {
  constructor(
  ...
  private async createVideo(input: SaveVideoInput) {
    ...
    const nestedCastMembersProps = await this.getCastMembersProps(input.cast_members_id);
  ...
  private async getCategoriesProps(categoriesIds: string[]): Promise<NestedCategoryConstructorProps[]> {
    ...
    if (notCategoriesExists.length > 0) {
      throw new EntityValidationError([
        {
          categories_id: notCategoriesExists.map((c) => new NotFoundError(c.id, Category).message),
        },
      ]);
    }
  ...
  private async getGenresProps(genresIds: string[]): Promise<NestedGenreConstructorProps[]> {
    const { exists: genresExists, not_exists: notGenresExists } = await this.genreRepo
      .ignoreSoftDeleted()
    ...
    if (notGenresExists.length > 0) {
      throw new EntityValidationError([
        {
          genres_id: notGenresExists.map((c) => new NotFoundError(c.id, Genre).message),
        },
      ]);
    }
  ...
  private async getCastMembersProps(castMembersIds: string[]): Promise<NestedCastMemberConstructorProps[]> {
    const { exists: castMembersExists, not_exists: notCastMembersExists } = await this.castMemberRepo
      .ignoreSoftDeleted()
    ...
    if (notCastMembersExists.length > 0) {
      throw new EntityValidationError([
        {
          cast_members_id: notCastMembersExists.map((c) => new NotFoundError(c.id, CastMember).message),
        },
      ]);
    }
