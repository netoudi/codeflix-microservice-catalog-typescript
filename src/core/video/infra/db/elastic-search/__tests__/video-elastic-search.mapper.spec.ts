...
describe('VideoElasticSearchMapper', () => {
  ...
  describe('toEntity', () => {
    it('should convert document to entity', () => {
      const result = VideoElasticSearchMapper.toEntity(video.video_id.id, videoDocument);
      expect(result).toEqual(video);

      videoDocument.deleted_at = new Date();
      video.deleted_at = videoDocument.deleted_at;

      const result2 = VideoElasticSearchMapper.toEntity(video.video_id.id, videoDocument);
      expect(result2).toEqual(video);

      videoDocument.categories[0].deleted_at = new Date();
      video.categories.get(videoDocument.categories[0].category_id)!.deleted_at =
        videoDocument.categories[0].deleted_at;

      videoDocument.genres[0].deleted_at = new Date();
      video.genres.get(videoDocument.genres[0].genre_id)!.deleted_at = videoDocument.genres[0].deleted_at;

      videoDocument.cast_members[0].deleted_at = new Date();
      video.cast_members.get(videoDocument.cast_members[0].cast_member_id)!.deleted_at =
        videoDocument.cast_members[0].deleted_at;

      const result3 = VideoElasticSearchMapper.toEntity(video.video_id.id, videoDocument);
  ...
  describe('toDocument', () => {
    it('should convert entity to document', () => {
      ...
      video.categories.get(videoDocument.categories[0].category_id)!.deleted_at = categoryDeletedAt;
      ...
      video.genres.get(videoDocument.genres[0].genre_id)!.deleted_at = genreDeletedAt;
      videoDocument.genres[0].deleted_at = genreDeletedAt;
      videoDocument.genres[0].is_deleted = true;

      const castMemberDeletedAt = new Date();

      video.cast_members.get(videoDocument.cast_members[0].cast_member_id)!.deleted_at = castMemberDeletedAt;
