...
import { VideoElasticSearchMapper } from '@/core/video/infra/db/elastic-search/video-elastic-search.mapper';
...
describe('VideoElasticSearchRepository Integration Tests', () => {
  ...
  test('should insert many entities', async () => {
    ...
    const { exists: foundVideos } = await repository.findByIds(videos.map((g) => g.video_id));
  ...
  it('should find a entity by id', async () => {
    ...
    await expect(repository.ignoreSoftDeleted().findById(entity.video_id)).resolves.toBeNull();
  });
  ...
  it('should return a videos list by ids', async () => {
    ...
    Promise.all([await repository.update(videos[0]), await repository.update(videos[1])]);

    const { exists: foundVideos2 } = await repository.ignoreSoftDeleted().findByIds(videos.map((g) => g.video_id));
  ...
  it('should return video id that exists', async () => {
    ...
    const existsResult2 = await repository.existsById([video.video_id, genreId1]);
    ...
    const existsResult3 = await repository.ignoreSoftDeleted().existsById([video.video_id]);
  ...
  it('should throw error on update when a entity not found', async () => {
    const entity = Video.fake().aVideoWithAllMedias().build();
    await expect(repository.update(entity)).rejects.toThrow(new NotFoundError(entity.video_id.id, Video));
  ...
  it('should throw error on delete when a entity not found', async () => {
    ...
    await expect(repository.delete(genreId)).rejects.toThrow(new NotFoundError(genreId.id, Video));

    const entity = Video.fake().aVideoWithAllMedias().build();
    await repository.insert(entity);
    entity.markAsDeleted();
    await repository.update(entity);
    expect(repository.ignoreSoftDeleted().delete(entity.video_id)).rejects.toThrow(
      new NotFoundError(entity.video_id.id, Video),
    );
  });
  ...
  describe('search method tests', () => {
    it('should only apply paginate and order by created desc when other params are null', async () => {
      ...
      const searchOutput2 = await repository.ignoreSoftDeleted().search(VideoSearchParams.create());
      ...
      const arrange = [
        ...
        },
        {
          params: VideoSearchParams.create({
            ...
            per_page: 2,
            filter: {
              categories_id: [nestedCategories[0].category_id.id, nestedCategories[1].category_id.id],
            },
          }),
          result: {
      ...
      const arrange = [
        ...
        },
        {
          params: VideoSearchParams.create({
            ...
            per_page: 2,
            filter: {
              genres_id: [nestedGenres[0].genre_id.id, nestedGenres[1].genre_id.id],
            },
          }),
          result: {
            items: [videos[4], videos[3]],
      ...
      const arrange = [
        ...
        },
        {
          params: VideoSearchParams.create({
            ...
            per_page: 2,
            filter: {
              cast_members_id: [nestedCastMembers[0].cast_member_id.id, nestedCastMembers[1].cast_member_id.id],
            },
          }),
          result: {
            items: [videos[4], videos[3]],
