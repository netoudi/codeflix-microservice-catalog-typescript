...
class AndCriteria implements ICriteria {
  ...
  applyCriteria(query: QueryDslQueryContainer): QueryDslQueryContainer {
    return this.criterias.reduce((acc, criteria) => criteria.applyCriteria(acc), query);
  }
}

async function main() {
  const container = new ElasticsearchContainer('elasticsearch:7.17.7').withReuse();
  ...
  const repository = new CategoryElasticSearchRepository(esClient, 'categories');
